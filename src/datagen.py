...
from src.utils.utils import get_boxes_from_mask, ResizeLongestSide
import json
import random
...
class TestingDataset(Dataset):
    ...
    def __init__(self, data_dir, image_size=Config['img_size'], mode='test', requires_name=True):
        """
        ...
        if isinstance(data_dir, list):
            ...
            for dir in data_dir:
                ...
                self.image_paths.extend([os.path.join(dir, mode, x) for x in dataset.values()])
                self.label_paths.extend([os.path.join(dir, mode, x) for x in dataset.keys()])
        else:
            dataset = json.load(open(os.path.join(data_dir, mode, f'label2image_{mode}.json'), "r"))
            self.image_paths = [os.path.join(data_dir, mode, x) for x in dataset.values()]
            self.label_paths = [os.path.join(data_dir, mode, x) for x in dataset.keys()]
...
class TrainingDataset(Dataset):
    ...
    def __init__(self, data_dir, image_size=Config['img_size'], mode='train', requires_name=False, mask_num=5):
        """
        ...
        if isinstance(data_dir, list):
            ...
            for dir in data_dir:
                ...
                self.image_paths.extend([os.path.join(dir, mode, k) for k in dataset.keys()])
                self.label_paths.extend([[os.path.join(dir, mode, v)] for v in dataset.values()])
        else:
            dataset = json.load(open(os.path.join(data_dir, mode, f'image2label_{mode}.json'), "r"))
            self.image_paths = [os.path.join(data_dir, mode, k) for k in dataset.keys()]
            self.label_paths = [[os.path.join(data_dir, mode, v)] for v in dataset.values()]
